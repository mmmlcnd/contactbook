...
class StudentController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        // --- ★修正: Log::debug に変更★ ---
        Log::debug("DEBUG: getStudentId returns ID: " . $studentId . " from session user_id: " . ($_SESSION['user_id'] ?? 'N/A'));
        // --------------------------
    ...
    public function showPastEntries(Request $request)
    {
        ...
        // --- ★修正: Log::debug に変更★ ---
        // ★修正: ログメッセージを @showPastEntries に修正
        Log::debug("DEBUG: StudentController@showPastEntries processing for studentId: " . $studentId);        // --------------------------
        ...
        // 前月を計算
        $displayCurrentMonth = $date->format('Y年n月');
        ...
        try {
            $results = Entry::getPastEntries($studentId, $currentMonth);

            // ★修正: 取得件数をログ出力 (デバッグ用)
            Log::debug("DEBUG: showPastEntries fetched " . $results->count() . " entries for month " . $currentMonth);
            ...
            Log::error("Database error in showPastEntries: " . $e->getMessage());
